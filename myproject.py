...
import isodate
import re
api_key = 'AIzaSyAf4LyNnXQOIYBtZzMcgy3R3oiICt2Y1Zg'
...
def data_to_mongo(input):
    client = pymongo.MongoClient("mongodb+srv://mugilvendan_v:mugilven@cluster0.yub8dul.mongodb.net/?retryWrites=true&w=majority")
    db = client["Youtube"]
...
def retrive(data):
    ...
    db = psycopg2.connect(host='localhost', user='postgres', password='mugilven', port=5432, database='youtubepro')
    exe = db.cursor()
...
with col1:
    st.markdown(":violet[Data Collection Zone]")
...
with col2:
    st.markdown(":violet[Data Migration Zone]")
...
st.markdown(":violet[Data Analysis Zone]")
